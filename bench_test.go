...
func benchmarkSendRecv(b *testing.B, sendSize, recvSize int) {
	...
	go func() {
		...
		case recvSize > sendSize:
			b.Errorf("bad test case; recvSize was: %d and sendSize was: %d, but recvSize must be <= sendSize!", recvSize, sendSize)
			return
...
func BenchmarkSendRecvParallelLarge(b *testing.B) {
	const payloadSize = 512 * 1024 * 1024
	benchmarkSendRecvParallel(b, payloadSize)
}
